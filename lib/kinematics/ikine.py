...
# ------------------------------------------------------------------------------
# """ FUNCTION: To Calculate 1 degrees of freedom (using Trigonometry) """
# ------------------------------------------------------------------------------
def remove_nan(list_):

    return [val for val in list_ if val==val]
# ------------------------------------------------------------------------------
# """ FUNCTION: To Calculate 1 degrees of freedom (using Trigonometry) """
# ------------------------------------------------------------------------------
def ik_1dof(x_axis=0, y_axis=0, z_axis=0):
    theta_1 = math.atan(y_axis / x_axis)  # eq1
    return remove_nan(theta_1)

# ------------------------------------------------------------------------------
# """ FUNCTION: To Calculate 2 degrees of freedom (using Trigonometry) """
# ------------------------------------------------------------------------------
def ik_2dof(x_axis=0, y_axis=0, z_axis=0):
    """
    Calculate 3 degrees of freedom using Trigonometry
    Note: only for 2 revolute joints
        __
       |__|
      _
    |_|
...
def ik_3dof(x_axis=0, y_axis=0, z_axis=0):
    ...
    phi_3_eq = (((r3 ** 2) - (const.L_2 ** 2) - (const.L_3 ** 2)) / (-2.0 * const.L_2 * const.L_3))
    phi_3 = np.arccos(make_val_in_range(phi_3_eq, min_range=-1, max_range=1))  # eq8
    theta_3 = np.pi - phi_3  # eq9

    return remove_nan([theta_1, theta_2, theta_3])


# ------------------------------------------------------------------------------
# """ FUNCTION: To Calculate 5 degrees of freedom  """
# ------------------------------------------------------------------------------
def ik_4dof(end_eff_direction_mat=const.end_eff_direction_mat, x_axis=0, y_axis=0, z_axis=0):
    """
    ...
    if z_axis is 0:
        z_axis = const.dist_from_cam
    theta_1, theta_2, theta_3 = ik_3dof(x_axis, y_axis, z_axis)

    fk_3dof = fkine.Fkine(const.DH_PT[:-2])  # removing last 2 row to get DH table for 3 joint
    ...
    return remove_nan([abs(theta_1), abs(theta_2), theta_3, theta_4])


# ------------------------------------------------------------------------------
# """ FUNCTION: To Calculate 5 degrees of freedom  """
# ------------------------------------------------------------------------------
def ik_5dof(end_eff_direction_mat=const.end_eff_direction_mat, x_axis=0, y_axis=0, z_axis=0):
    """
    Calculate 5 degrees of freedom
    ...
    # fk_3dof = fkine.Fkine(const.PT_3dof)  # removing last 2 row to get DH table for 3 joint
    fk_3dof = fkine.Fkine(const.DH_PT[:-2])  # removing last 2 row to get DH table for 3 joint
    ...
    #                 theta_5=theta_5
    #                 )
    return remove_nan([abs(theta_1), abs(theta_2), theta_3, theta_4, theta_5])


# -----------------------------------------------
IK_FUNC_DICT = {1: ik_1dof,
                2: ik_2dof,
                3: ik_3dof,
                4: ik_4dof,
                5: ik_5dof
...
if __name__ == "__main__":
    ...
    print(ik_5dof(R0_5, 50.0, 60.0, 60.0))
